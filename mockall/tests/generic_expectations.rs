...
mod generic_expectation {
    ...
    fn checkpoint_ok() {
        expectation!{
            pub fn foo<T>(&self, t: T) -> u32 { let (p: &T) = (&t); }
        }
    ...
    #[test]
    fn generic_nothing() {
        expectation!{ pub fn foo<T>(&self) -> () { let () = (); } }
    ...
    fn generic_return() {
        expectation!{
            pub fn foo<T>(&self, x: i32) -> T { let (p: &i32) = (&x); }
        }
        let mut e = foo::GenericExpectations::default();
    ...
    fn reference_arguments() {
        expectation!{
            pub fn foo<T>(&self, t: T, x: &i16) -> u32 {
                let (pt: &T, px: &i16) = (&t, x);
            }
        }
        let mut e = foo::GenericExpectations::default();
    ...
    fn multiple_methods() {
        ...
        expectation!{
            pub fn bar<T>(&self, t: T) -> u32 { let (p: &T) = (&t); }
        }
    ...
    /// Unlike Mockers, calls should use the oldest matching expectation, ipub f
    ...
    #[test]
    fn one_match() {
        expectation!{ pub fn foo<T>(&self, t: T) -> T { let (p: &T) = (&t); } }
...
mod generic_ref_mut_expectation {
    ...
    fn generic_argument() {
        expectation!{
            pub fn foo<T>(&mut self, t: T) -> &mut u32 { let (p: &T) = (&t); }
        }
        let mut e = foo::GenericExpectations::default();
    ...
    fn generic_return() {
        expectation!{
            pub fn foo<T>(&mut self, x: i32) -> &mut T { let (p: &i32) = (&x); }
        }
        let mut e = foo::GenericExpectations::default();
    ...
    fn reference_arguments() {
        expectation!{
            pub fn foo<T>(&mut self, t: T, x: &i16) -> &mut u32 {
                let (pt: &T, px: &i16) = (&t, x);
            }
        }
        let mut e = foo::GenericExpectations::default();
...
/// Mocking generic static methods requires using globals
mod static_method {
    use std::sync::Mutex;
    use super::*;

    expectation!{
        pub fn foo<T>(t: T, x: &i16) -> u32 {
            let (pt: &T, px: &i16) = (&t, x);
        }
    }
    mockall::lazy_static! {
        static ref GLOBAL_EXPECTATIONS: Mutex<foo::GenericExpectations> =
            Mutex::new(foo::GenericExpectations::new());
    }
    ...
    fn t() {
        {
            let mut eguard = {
                ...
                foo::GenericExpectationGuard::new(guard)
            };
            eguard.returning(|_: i32, _: &i16| 42u32);
        }

        let x = -6i16;
        assert_eq!(42, GLOBAL_EXPECTATIONS.lock().unwrap().call::<i32>(5, &x));
    }
}
