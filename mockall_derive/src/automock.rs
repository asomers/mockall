...
mod t {
    ...
    fn generic_trait() {
        ...
        mod __mock_Foo_Foo {
            use super:: * ;
            ::mockall::expectation! { fn foo<T>(&self) -> () { let () = (); } }
        }
    ...
    fn impl_trait() {
        ...
        mod __mock_Foo {
            use super:: * ;
            ::mockall::expectation! {
                fn foo< >(&self) -> Box<dyn Debug + Send> { let () = (); }
            }
        }
        pub struct MockFoo {
        ...
        impl MockFoo {
            ...
            fn expect_foo(&mut self) -> &mut __mock_Foo::foo::Expectation
            {
                self.foo.expect()
            }
            pub fn checkpoint(&mut self) {
    ...
    fn impl_trait_on_generic() {
        ...
        mod __mock_Bar_Foo {
            use super:: * ;
            ::mockall::expectation!{
                fn foo<T>(&self, x: u32) -> i64 { let (p1: &u32) = (&x); }
            }
        }
    ...
    fn impl_trait_with_associated_types() {
        ...
        mod __mock_Foo_Iterator {
            use super:: * ;
            ::mockall::expectation!{
                fn next< >(&mut self) -> Option<u32> { let () = (); }
            }
        }
    ...
    fn mutable_argument() {
        ...
        mod __mock_Foo {
            use super:: * ;
            ::mockall::expectation!{
                fn foo< >(&self, mut x: u32) -> () { let (p1: &u32) = (&x); }
            }
            ::mockall::expectation!{
                fn bar< >(mut self) -> () { let () = (); }
            }
    ...
    fn static_impl_trait_constructor() {
        ...
        mod __mock_A_A {
            use super:: * ;
            ::mockall::expectation!{
                fn new< >() -> Box<Future<Item=MockA, Error=()> > {
                    let () = ();
                }
            }
        }
    ...
    fn where_clause_on_struct() {
        let desired = r#"
            ...
            mod __mock_Foo {
                use super:: * ;
                ::mockall::expectation!{
                    fn foo<T>(&self, x: T) -> T { let (p1: &T) = (&x); }
                }
            }
